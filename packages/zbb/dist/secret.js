 ...
 */
function secretsDir(slot) {
    return join(slot.stateDir, 'secrets');
}
/**
 * Ensure secrets directory exists
 */
function ensureSecretsDir(slot) {
 ...
 */
function findSecretFile(dir, name) {
 ...
 */
export async function handleSecret(args, slot) {
    const sub = args[0];
    switch (sub) {
        case 'create':
 ...
 */
async function secretCreate(args, slot) {
    const name = args[0];
 ...
 */
async function secretUpdate(args, slot) {
    const name = args[0];
    if (!name) {
 ...
 */
async function secretDelete(args, slot) {
    const name = args[0];
    if (!name) {
