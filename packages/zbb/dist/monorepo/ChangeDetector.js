 ...
 */
function resolveBaseRef(repoRoot, overrideBase) {
    if (overrideBase)
        return overrideBase;
    ...
    if (branch === 'main' || branch === 'master') {
        ...
        try {
            const lastStampCommit = git(['log', '-1', '--format=%H', '--', 'gate-stamp.json'], repoRoot);
            if (lastStampCommit)
                return lastStampCommit;
        }
 ...
 */
function mapFilesToPackages(changedFiles, graph) {
 ...
 */
export function detectChanges(repoRoot, graph, options) {
