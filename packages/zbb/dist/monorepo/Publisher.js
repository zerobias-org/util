...
import { GateStampResult, readGateStamp, validatePackageStamp, writeGateStamp, computeSourceHash, } from './GateStamp.js';
 ...
 */
function isVersionPublished(packageName, version, registry) {
    try {
 ...
 */
function getLatestPublishedVersion(packageName, registry) {
    try {
 ...
 */
function resolvePublishVersion(pkg, registry) {
 ...
 */
function patchPackageJsonVersion(pkg, newVersion) {
 ...
 */
function updateDependencyVersion(pkg, depName, newVersion) {
    const pkgJsonPath = join(pkg.dir, 'package.json');
    const content = readFileSync(pkgJsonPath, 'utf-8');
...
function computeSourceHashForPublish(pkg, config) {
