 ...
 */
export function isMonorepo(repoRoot) {
...
function runMonorepoPreflight(command, config) {
    const requirements = [...MONOREPO_PREFLIGHT];
    if (command === 'publish')
        requirements.push(...PUBLISH_PREFLIGHT);
 ...
 */
export function isMonorepoCommand(command) {
    return MONOREPO_COMMANDS.has(command);
}
/**
 * Main monorepo command handler.
 */
export async function handleMonorepo(command, args, repoRoot) {
