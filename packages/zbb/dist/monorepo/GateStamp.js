...
// ── Types ────────────────────────────────────────────────────────────
export var GateStampResult;
(function (GateStampResult) {
    /** sourceHash + tasks + test counts all match — skip all gate tasks */
    GateStampResult["VALID"] = "VALID";
    /** source OK but test count mismatch — need to re-run tests */
    GateStampResult["TESTS_CHANGED"] = "TESTS_CHANGED";
    /** hash mismatch or task failure — full gate needed */
    GateStampResult["INVALID"] = "INVALID";
    /** no entry for this package */
    GateStampResult["MISSING"] = "MISSING";
})(GateStampResult || (GateStampResult = {}));
 ...
 */
export function computeSourceHash(pkg, config) {
 ...
 */
export function countExpectedTests(testDir) {
...
export function readGateStamp(repoRoot) {
...
export function writeGateStamp(repoRoot, stamp) {
    const stampPath = join(repoRoot, STAMP_FILE);
 ...
 */
export function validatePackageStamp(pkg, stamp, config) {
    if (!stamp)
        return GateStampResult.MISSING;
 ...
 */
export function validateStamp(affectedPackages, graph, repoRoot, config) {
 ...
 */
export function isStampValid(affectedPackages, graph, repoRoot, config) {
 ...
 */
export function buildPackageStampEntry(pkg, config, taskResults, testResults) {
    return {
        version: pkg.version,
