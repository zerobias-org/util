...
export async function handleDataloader(args: string[]): Promise<void> {
  ...
  for (const v of pgVars) {
    if (slotEnv[v]) childEnv[v] = slotEnv[v];
  }

  // Also inject NPM_TOKEN if in slot (needed for registry access)
  if (slotEnv.NPM_TOKEN) childEnv.NPM_TOKEN = slotEnv.NPM_TOKEN;
