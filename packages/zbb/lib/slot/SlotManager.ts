...
export class SlotManager {
  ...
  static async create(name: string, options: CreateOptions = {}): Promise<Slot> {
    ...
    // 6. Collect inherited vars from process.env.
    //
    // Always: vars declared `source: env`.
    // CI mode: ALSO any declared var (regardless of source) whose value is
    //          already in process.env. This lets vault-action populate
    //          NEON_API_KEY etc. before slot create runs, and the slot
    //          inherits those values directly without re-fetching from Vault.
    ...
    if (ciMode) {
      // In CI, snapshot any declared var that's already in process.env. Skip
      // ones already inherited above. Skip ports/secrets — those are
      // allocated/generated by the slot, not provided by the parent shell.
      for (const v of scanned) {
        if (inherited.has(v.name)) continue;
        if (v.declaration.type === 'port' || v.declaration.type === 'secret') continue;
        const value = process.env[v.name];
        if (value) inherited.set(v.name, value);
      }
    }
    ...
    // Inherited (declared `source: env`, plus everything snapshotted from
    // process.env in CI mode)
    const inheritedNameSet = new Set<string>();
    for (const v of inheritedVars) {
      ...
      if (value) {
        ...
        inheritedNameSet.add(v.name);
      }
    }
    if (ciMode) {
      // Anything in `inherited` that wasn't already written above came from
      // the CI snapshot pass. Look up the scan entry to recover the source
      // location for the manifest.
      for (const [name, value] of inherited) {
        if (inheritedNameSet.has(name)) continue;
        const scanEntry = scanned.find(s => s.name === name);
        env.set(name, value);
        manifest.set(name, {
          source: scanEntry?.source ?? 'ci-env',
          type: 'inherited',
