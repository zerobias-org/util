...
All order types may be provided a custom json context that is set by the customer.
   ...
   * @example
   * ```typescript
   * const result = await api.list(1, 1, null, null, null, null, 'orderBy', 'pageToken');
   * ```
   * @openapi operation=listOrders path=/orders method=GET
   * @memberof OrderApi
   */
  list(pageNumber?: number, pageSize?: number, orderStatus?: OrderStatusParam, orderType?: OrderTypeDef, priority?: PriorityDef, sort?: SortParamDef, orderBy?: OrderApi.OrderByEnumDef, pageToken?: string): Promise<PagedResults<Order>>;
...
All order types may be provided a custom json context that is set by the customer.
   ...
   * @openapi operation=listOrders path=/orders method=GET
   */
  public async list(pageNumber: number = 1, pageSize: number = 50, orderStatus?: OrderStatusParam, orderType?: OrderTypeDef, priority?: PriorityDef, sort?: SortParamDef, orderBy: OrderApi.OrderByEnumDef = OrderApi.OrderByEnum.Created as OrderApi.OrderByEnumDef, pageToken?: string): Promise<PagedResults<Order>> {
    if (pageNumber) {
